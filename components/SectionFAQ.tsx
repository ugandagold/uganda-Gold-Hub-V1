                ...
                <div key={faq.id || index} className="py-6 group">
                  ...
                  >
                    ...
                    <span className={`flex items-center justify-center w-8 h-8 rounded-full transition-colors ${openIndex === index ? 'bg-amber-500 text-white' : 'text-stone-300'}`}>
                      {openIndex === index ? <ArrowUp size={14} aria-hidden="true" /> : <ArrowDown size={14} aria-hidden="true" />}
                    </span>
                  </button>
                  <div 
                    id={`faq-answer-${index}`}
                    ...
                    className={`overflow-hidden transition-all duration-500 ease-in-out ${openIndex === index ? 'max-h-48 opacity-100 mt-4' : 'max-h-0 opacity-0'}`}
                  >
